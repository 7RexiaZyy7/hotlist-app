...
export function buildTopicAnalysisQuery(topic: string): string {
  return `请深度分析以下热点话题的创作价值，输出结构化的分析报告。严格按照下面的 Markdown 格式输出，不要省略任何部分。
...
export function buildInspirationPrompt(topic: string, angles: string[]): string {
  return `你是一个资深社交媒体文案策划专家。用户正在创作关于「${topic}」的文案，以下是他们选择的切入角度：${angles.join('、')}。

请从不同角度出发，给出 3-5 条能激发灵感的文案方向建议。每条建议先写一段话（20-60字），然后在末尾用【】标注这条建议更适合哪个切入角度。

要求：
- 每条建议必须实用、具体，不能泛泛而谈
- 覆盖多个角度，不要全集中在一个角度
- 每条末尾的【角度名】必须从用户提供的角度中选择
- 直接输出建议列表，不要前言后语

输出格式示例：
在快节奏的生活中，人们越来越渴望简单纯粹的快乐，这正是你用情感共鸣打动受众的切入点【情感共鸣】
最新数据显示该领域用户增长迅猛，现在入场仍有红利期，建议结合具体案例和数据来制造紧迫感【热点追踪】`;
}
