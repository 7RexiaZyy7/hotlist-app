...
const angles = [
  ...
  { id: 'knowledge', label: '知识科普', icon: BookOpen, desc: '干货拆解 + 通俗解释，建立信�? },
  { id: 'opinion', label: '观点评论', icon: MessageSquare, desc: '鲜明立场 + 论据，引发站队讨�? },
  { id: 'trend', label: '热点追踪', icon: Flame, desc: '借势当前热点，吃流量' },
  { id: 'future', label: '未来趋势', icon: Rocket, desc: '前瞻视角，吸引关注趋势的用户' },
  { id: 'analysis', label: '深度分析', icon: Search, desc: '多角度拆解，凸显专业�? },
  { id: 'funny', label: '趣味解读', icon: Smile, desc: '段子/�?反讽，病毒传播潜力高' },
  { id: 'story', label: '故事叙述', icon: Book, desc: '真实故事/经历，最强代入感' },
  { id: 'suggestion', label: '实用建议', icon: Lightbulb, desc: '可操作的清单/方法论，收藏率高' },
  { id: 'question', label: '提问互动', icon: HelpCircle, desc: '结尾设问，引导评�? },
  { id: 'comparison', label: '对比分析', icon: GitCompare, desc: 'A vs B 对比，结构清晰易�? },
];

const platformStyles = [
  { id: 'xiaohongshu', label: '小红书版', description: '适合小红书平台风�? },
  { id: 'douyin', label: '抖音脚本', description: '适合抖音短视�? },
  { id: 'gongzhonghao', label: '公众号版', description: '适合公众号文�? },
  { id: 'weibo', label: '微博�?, description: '适合微博短文�? },
];
...
export function ContentForge() {
  ...
  const handleGenerate = async () => {
    if (forgeMode === 'create') {
      ...
      if (selectedAngles.length === 0) { showToast('请至少选择一个创作角�?, 'warning'); return; }
    } else if (forgeMode === 'deconstruct') {
      if (!selectedTopic.trim()) { showToast('请输入要拆解的话题或文案', 'warning'); return; }
    } else if (forgeMode === 'rewrite') {
      if (!rewriteSource.trim()) { showToast('请粘贴要改写的文�?, 'warning'); return; }
    }
    ...
    try {
      if (forgeMode === 'deconstruct') {
        ...
        const style = selectedTopic.trim() || '保持原风�?;
        ...
        if (parsed.length === 0) {
          ...
          showToast('AI 返回内容格式异常，请换个角度或话题再�?, 'error');
          return;
        }
        ...
        showToast(`已生�?${parsed.length} 种角度的文案`);
      }
  ...
  const handleConvert = async (platform: string) => {
    ...
    try {
      ...
      const query = `请将以下 ${generatedCopies.length} 段文案，同时转换为以�?4 个平台版本：${platformStyles.map(p => p.label).join('�?)}�?{profileSection}

要求每个平台版本按以下顺序输出，用�?=平台�?=」作为分隔：
==${platformStyles.map(p => p.label).join('==\n==') + '=='}

原文�?${copiesText}

输出格式（严格遵守）�?==${platformStyles[0].label}==
[${platformStyles[0].label}版本的角�?文案]
[${platformStyles[0].label}版本的角�?文案]
...
==${platformStyles[1].label}==
[${platformStyles[1].label}版本的角�?文案]
...`;
      ...
      showToast(`已生�?${platformLabels} 4 个平台版本`);
    } catch {
    ...
    <div className="p-4 md:p-6 pb-24 md:pb-6 max-w-shell mx-auto">
      <div className="mb-6">
        ...
        <p className="text-body-sm text-text-secondary">
          ...
          {forgeMode === 'rewrite' && '输入现有文案，AI 用你的风格重新改�?}
        </p>
      </div>
      ...
      <div className="card p-4 mb-6">
        {forgeMode === 'create' ? (
          <div className="flex gap-2">
            ...
            />
            <button onClick={() => setShowAssetLibrary(true)} className="btn-ghost shrink-0" title="素材�?>
            ...
            <button onClick={handleGenerate} disabled={isGenerating} className={clsx('btn-primary', isGenerating && 'opacity-40 cursor-not-allowed')}>
              {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />生成�?..</> : <><Sparkles className="w-4 h-4" />生成文案</>}
            </button>
          </div>
        ) : forgeMode === 'rewrite' ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleGenerate} disabled={isGenerating || !rewriteSource.trim()} className={clsx('btn-primary', (isGenerating || !rewriteSource.trim()) && 'opacity-40 cursor-not-allowed')}>
                {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />改写�?..</> : <><Sparkles className="w-4 h-4" />开始改�?/>}
              </button>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <div className="flex justify-end">
              <button onClick={handleGenerate} disabled={isGenerating || !selectedTopic.trim()} className={clsx('btn-primary', (isGenerating || !selectedTopic.trim()) && 'opacity-40 cursor-not-allowed')}>
                {isGenerating ? <><RefreshCw className="w-4 h-4 animate-spin" />拆解�?..</> : <><Sparkles className="w-4 h-4" />开始拆�?/>}
              </button>
            </div>
          </div>
        )}
        ...
        {forgeMode === 'create' && (userProfile?.niche || userProfile?.audience || userProfile?.style ? (
          ...
          <div className="mt-3 flex items-center gap-1.5 text-caption text-text-tertiary">
            ...
            <span>没填「创作者档案」？</span>
            <button onClick={() => setActivePage('profile')} className="text-accent hover:text-accent-hover">�?30 秒填一下，AI 文案会对你口味的 2 �?/button>
          </div>
        ))}
      </div>
      ...
      {forgeMode === 'create' && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-caption text-text-tertiary">已�?{selectedAngles.length}/5</span>
          </div>
      ...
      {isGenerating ? (
        <LoadingState steps={
          forgeMode === 'deconstruct'
            ...
            : forgeMode === 'rewrite'
              ? ['正在理解原文...', '正在风格转换...', '正在生成改写�?..']
              : ['正在分析热点话题...', '正在构思创作角�?..', '正在生成爆款文案...', '正在优化内容质量...']
        } />
        ...
        <div ref={resultsRef} className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => navigator.clipboard.writeText(deconstructResult).then(() => showToast('已复�?))}
        ...
        <div className="card p-8 text-center">
          ...
          {forgeMode === 'create' ? (
            <>
              <h3 className="text-body font-semibold text-text-primary mb-1">选个话题开始创�?/h3>
              <p className="text-body-sm text-text-secondary mb-6">从下�?3 个热门方向选一个，AI 立刻生成爆款文案</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-3xl mx-auto">
                {[
                  { topic: '35岁职场危�?, angle: 'emotional', label: '35 岁危�?, desc: '情感共鸣', emoji: '😰' },
            ...
            <>
              ...
              <p className="text-body-sm text-text-secondary">粘贴你的文案或输入话题，AI 会分析其创作逻辑和可复用的模�?/p>
            </>
          ) : (
            <>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={() => {
                  ...
                  navigator.clipboard.writeText(text).then(() => showToast('已复制全部文�?));
                }}
              ...
              title="一次生�?4 个平台版本（仅消�?1 次额度）"
            >
              <Wand2 className="w-3.5 h-3.5" />
              {convertingPlatform ? '转换�?..' : Object.keys(convertedCopies).length > 0 ? '已生成多平台版本 · 重新转换' : '一键转换为多平台版�?}
            </button>
          </div>
          ...
          {selectedForCompare.size >= 2 && !showComparison && (
            <div className="flex justify-end mb-2">
              ...
              >
                ...
                对比 {selectedForCompare.size} 个版�?              </button>
            </div>
          )}
                ...
                return (
                  <div key={idx} className="bg-bg-surface border border-border rounded-lg p-4">
                    ...
                    <div className="mt-3 pt-2 border-t border-border text-caption text-text-tertiary flex items-center justify-between">
                      <span>{(body || copy.content).length} �?/span>
                      <button
...
function parseAllPlatformCopies(
  ...
  // 如果没解析出任何平台，回退到原�?  if (Object.keys(result).length === 0) {
